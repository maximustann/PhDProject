...
energyIncreasing <- function(base, run, numOfContainer){
	...
	labels <- c(rep('aaaaaaaaaaaaa', numOfContainer+1), rep('bbbbbb', numOfContainer+1), rep('ccccc', numOfContainer+1))
	#labels <- factor(c(rep('aaaaaaaaaaaa', numOfContainer+1), rep('bbbbb', numOfContainer+1)))
	#data <- data.frame(label=labels, mark=mark, SlopeOfIncrement=c(justFitSub, evoOneAverage, evoTwoAverage))
	data <- data.frame(label=labels, mark=mark, SlopeOfIncrement=c(justFitSub, evoOneAverage, evoTwoAverage))


	p <- ggplot(data=data, aes(x=mark, y=SlopeOfIncrement)) + 
		geom_line(aes(col=label), size=1.5) + geom_area(aes(fill=label), alpha=.4, position="identity") + scale_color_manual(values=c('#fa9d22', '#ffff7f', 'cyan')) + labs(x='Container', y='Energy consumption') + 
