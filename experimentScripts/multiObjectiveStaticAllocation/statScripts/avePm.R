...
avePm <- function(testCase = 1){
	...
	p <- ggplot(data, aes(x=x, y=y)) + geom_bar(stat="identity", width = 0.3, aes(fill=algorithm), position=position_dodge()) + scale_x_discrete(labels=c("5", "7", "10")) + scale_fill_brewer(palette="Set1") + xlab("Number of VM types") + ylab("Average number of used PMs") + theme(axis.text = element_text(size = 40, face="bold"), legend.text = element_text(size=45, face="bold"), legend.title=element_text(""), axis.title=element_text(size=45, face="bold"), legend.position=c(0.5, 0.2))
